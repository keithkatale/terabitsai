...
import { ShieldCheck, ArrowRight, Check, Sparkles, Loader2, Landmark, AlertCircle } from "lucide-react";
import { useAppAccount, type TradingMode } from "@/contexts/app-account-context";
...
export function TradeConfirmationWidget({
  ...
  const { tradingMode: accountMode } = useAppAccount();
  const tradingMode = modeProp ?? accountMode;
  const isLive = tradingMode === "live";

  const [dragPercent, setDragPercent] = useState(0);
  const [status, setStatus] = useState<"idle" | "dragging" | "executing" | "confirmed" | "failed">("idle");
  ...
  useEffect(() => {
    ...
    const onFailed = (e: Event) => {
      ...
      setStatus("failed");
      setDragPercent(0);
      setErrorMessage(detail?.message ?? "Trade execution failed");
    };

    window.addEventListener("quant-trade-executed", onExecuted);
    window.addEventListener("quant-trade-failed", onFailed);
    return () => {
      window.removeEventListener("quant-trade-executed", onExecuted);
      window.removeEventListener("quant-trade-failed", onFailed);
    };
  }, []);
  ...
  const ticketTitle = isLive ? "Live Trade Ticket" : "Paper Trade Ticket";
  const swipeLabel = isLive
    ? `Swipe right to execute LIVE ${isBuy ? "BUY" : "SELL"}`
    : `Swipe right to execute paper ${isBuy ? "BUY" : "SELL"}`;
  const successLabel = isLive ? "Live Trade Executed!" : "Paper Trade Executed!";
  const executingLabel = isLive
    ? "Submitting order to Capital.com..."
    : "Confirming paper trade...";
    ...
    <div className="w-full bg-zinc-950/50 backdrop-blur-xl border border-zinc-900 rounded-2xl p-4 flex flex-col gap-4 shadow-xl animate-fade-in my-3 text-left">
      <div className="flex items-center justify-between border-b border-zinc-900/40 pb-2.5">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-bold text-zinc-100 uppercase tracking-wider">{ticketTitle}</span>
        </div>
        <div className="flex items-center gap-1.5">
          <div
            className={cn(
              "text-[9px] px-1.5 py-0.5 rounded-md border font-extrabold uppercase tracking-wide",
              isLive
                ? "border-emerald-500/20 bg-emerald-950/20 text-emerald-400"
                : "border-cyan-500/10 bg-indigo-950/10 text-cyan-400",
            )}
          >
      ...
      {status === "confirmed" ? (
        ...
        <div className="flex flex-col gap-2">
          <div className="w-full min-h-11 bg-rose-500/10 border border-rose-500/20 text-rose-400 rounded-xl flex items-center justify-center gap-2 font-bold text-xs select-none uppercase tracking-wider px-3 py-2">
            <AlertCircle className="size-4 shrink-0" />
            <span>{errorMessage ?? "Trade failed"}</span>
          </div>
          <button
            type="button"
            onClick={() => {
            ...
            className="text-[10px] font-bold uppercase tracking-wider text-zinc-400 hover:text-zinc-200"
          >
            Try again
          </button>
        </div>
